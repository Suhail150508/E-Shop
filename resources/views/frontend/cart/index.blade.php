...
@section('title', __('common.shopping_cart'))
            ...
            <div class="cart-page-header">
                <div>
                    <div class="cart-breadcrumb">
                        ...
                        <span> / </span>
                        <span>{{ $cartBreadcrumb ?? __('common.cart_title') }}</span>
                    </div>
                    <h1 class="cart-title">{{ $cartTitle ?? __('common.shopping_cart') }}</h1>
                    <p class="cart-subtitle">{{ $cartSubtitle ?? __('common.cart_subtitle_review') }}</p>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            @if($items->isEmpty())
                                <div class="cart-empty-message">
                                    <p class="mb-3">{{ __('common.empty_cart') }}</p>
                                    ...
                                    <table class="table align-middle cart-items-table mb-0">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-end">{{ __('common.subtotal') }}</th>
                                                <th></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <div class="d-flex align-items-center cart-product-info">
                                                            <img src="{{ getImageOrPlaceholder($item['image'], '80x80') }}" alt="{{ $item['name'] }}" class="rounded me-3 cart-product-thumb" onerror="this.src='{{ asset('backend/images/placeholder.svg') }}'">
                                                    ...
                                                    <td class="text-center">
                                                        @if(isset($item['discount_price']) && $item['discount_price'] !== null && $item['discount_price'] > 0 && $item['discount_price'] < $item['price'])
                                                            <div class="cart-price-current">{{ format_price((float) $item['unit_price']) }}</div>
                                                            <div class="text-muted text-decoration-line-through small cart-price-old">{{ format_price((float) $item['price']) }}</div>
                                                        @else
                                                            <div class="fw-semibold">{{ format_price((float) $item['unit_price']) }}</div>
                                                        @endif
                                                    </td>
                                                    <td class="text-center">
                    ...
                    <div class="card border-0 shadow-sm order-summary-card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <span class="order-summary-title">{{ __('common.order_summary') }}</span>
                            </div>
