...
@section('page_title', __('Add Unit'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="h4 fw-bold mb-1">{{ __('Add Unit') }}</h2>
        <p class="text-muted mb-0">{{ __('Create a new product unit.') }}</p>
    </div>
        ...
        <form action="{{ route('admin.units.store') }}" method="POST">
            ...
            <div class="row g-4">
                <div class="col-lg-8">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-lg @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" placeholder="{{ __('e.g. kg, pc, box') }}" required>
                        @error('name')
            ...
            <div class="d-flex justify-content-start gap-2 mt-4 pt-3 border-top">
                <button type="submit" class="btn btn-primary px-4">
                    <i class="bi bi-save me-1"></i> {{ __('Save Unit') }}
                </button>
