                        ...
                        <tr>
                            ...
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="{{ route('admin.language.edit', $language) }}" class="btn btn-sm btn-secondary-soft">
                                        <i class="fas fa-edit"></i> {{ __('Edit') }}
                                    </a>
                                    <form action="{{ route('admin.language.destroy', $language) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure?') }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger-soft" {{ $language->is_default ? 'disabled' : '' }}>
