...
@section('content')
@php
    $title = setting('auth_reset_title', __('common.reset_password_title'));
    $subtitle = setting('auth_reset_subtitle', __('common.set_new_password_subtitle'));
    ...
    <div class="row g-0" style="min-height: 100vh;">
        ...
        <div class="col-lg-6 d-none d-lg-block position-relative overflow-hidden bg-light">
            ...
            <div class="position-absolute bottom-0 start-0 p-5 text-white w-100" style="background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);">
                <h2 class="display-5 fw-bold mb-3">{{ __('common.secure_account') }}</h2>
                <p class="lead mb-0 text-white-50">{{ __('common.secure_account_text') }}</p>
            </div>
        </div>
            ...
            <div class="w-100 p-4 p-md-5" style="max-width: 550px;">
                ...
                <form method="POST" action="{{ route('password.update') }}">
                    ...
                    <div class="form-floating mb-3 position-relative">
                        <input type="password" class="form-control @error('password') is-invalid @enderror" id="password" name="password" required placeholder="{{ __('common.enter_new_password') }}">
