...
@section('content')
@php
    $page = \App\Models\Page::where('slug', 'auth-reset-password')->first();
    $title = $page ? $page->title : __('Reset Password');
    $subtitle = $page ? $page->content : __('Create a new password for your account.');
    $image = $page && $page->image ? (filter_var($page->image, FILTER_VALIDATE_URL) ? $page->image : asset($page->image)) : 'https://images.unsplash.com/photo-1487017159836-4e23ece2e4cf?ixlib=rb-1.2.1&auto=format&fit=crop&w=1351&q=80';
@endphp
