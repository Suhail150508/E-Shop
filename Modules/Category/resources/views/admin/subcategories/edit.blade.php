...
@section('page_title', __('Edit Subcategory'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="h4 fw-bold mb-1">{{ __('Edit Subcategory') }}</h2>
        <p class="text-muted mb-0">{{ __('Update subcategory details.') }}</p>
    </div>
        ...
        <form action="{{ route('admin.subcategories.update', $subcategory->id) }}" method="POST">
            ...
            <div class="row g-4">
                <div class="col-lg-8">
                    <div class="mb-3">
                        ...
                        <select class="form-select form-select-lg @error('parent_id') is-invalid @enderror" id="parent_id" name="parent_id" required>
                            ...
                            @foreach($parents as $id => $parentName)
                                <option value="{{ $id }}" {{ (string) old('parent_id', $subcategory->parent_id) === (string) $id ? 'selected' : '' }}>
                                    {{ $parentName }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-lg @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $subcategory->name) }}" required>
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                ...
                <button type="submit" class="btn btn-primary px-4">
                    <i class="bi bi-save me-1"></i> {{ __('Update Subcategory') }}
                </button>
            </div>
        </form>
    </div>
