        ...
        <div class="col-md-6 col-lg-4">
            <div class="card bg-primary text-white border-0 shadow-sm overflow-hidden h-100 position-relative">
                <div class="card-body position-relative z-1">
                    <h6 class="text-white-50 mb-2">{{ __('Current Balance') }}</h6>
                    <h2 class="mb-0 fw-bold">{{ $settings->currency_symbol ?? '$' }}{{ number_format($user->wallet_balance, 2) }}</h2>
                    @if($pendingBalance > 0)
                        <small class="text-white-50 mt-2 d-block">
                            <i class="fas fa-clock me-1"></i>
                            {{ __('Pending: ') }} {{ $settings->currency_symbol ?? '$' }}{{ number_format($pendingBalance, 2) }}
                        </small>
                    @endif
                </div>
        ...
        <div class="col-md-6 col-lg-8">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div>
                        <h6 class="text-muted mb-1">{{ __('Total Deposited') }}</h6>
                    ...
                    <div>
                        <h6 class="text-muted mb-1">{{ __('Total Spent') }}</h6>
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th class="border-top-0">{{ __('Amount') }}</th>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($transaction->type === 'credit')
                                    <span class="badge bg-success-subtle text-success">{{ __('Credit') }}</span>
                                @else
                                    <span class="badge bg-danger-subtle text-danger">{{ __('Debit') }}</span>
                                @endif
                            </td>
            ...
            <div class="modal-content border-0 shadow">
                <form action="{{ route('customer.wallet.deposit') }}" method="POST">
                    ...
                    <div class="modal-header bg-light">
                        <h5 class="modal-title">{{ __('Add Funds to Wallet') }}</h5>
                    ...
                    <div class="modal-body p-4">
                        <div class="mb-4">
                            ...
                            <div class="form-text">{{ __('Maximum deposit limit: :limit', ['limit' => ($settings->currency_symbol ?? '$') . number_format($depositLimit, 2)]) }}</div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            @if(empty($gateways))
                                <div class="alert alert-warning">
                                    <i class="fas fa-exclamation-triangle me-2"></i> {{ __('No payment methods available.') }}
                                </div>
                            @else
                                <div class="row g-2">
                    ...
                    <div class="modal-footer bg-light">
                        ...
                        <button type="submit" class="btn btn-primary px-4" {{ empty($gateways) ? 'disabled' : '' }}>{{ __('Proceed to Payment') }}</button>
                    </div>
                </form>
            </div>
        </div>
